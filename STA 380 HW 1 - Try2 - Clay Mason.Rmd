...
green2 = read.csv('/Users/claytonmason/Archive/greenbuildings.csv')
#dim(green2)
...
```
**look at initial value of the weights**
The first portfolio contains equal weights
...
```
```{r}
print("use bootstrap resampling to estimate the 4-week (20 trading day) value at risk of each of your three portfolios at the 5% level.")
print("Aggressive")
print(Var_aggressive)
```
```{r}
...
social = read.csv('/Users/claytonmason/GitHub/STA_380_Clay/Data/social_marketing.csv', header=TRUE)
...
```
**Data Cleaning**
I removed the porn bots by considering anything with 2 or more in this category. I also removed spam bots, but the porn bots had the bigger impact. 
Now we have 7,456 observations after previously having 7,882. 
There are 37 variables in the file.
```{r message=FALSE}
#cut spam 'bots'
...
```
**Data Exploration**
Simple correlation matrix to see if there are any obvious relationships to dig into. 
Simple Top 10 Table and bar plot to see the highest frequency categories. 
The most common include the miscellaneous chatter category. 
```{r echo=FALSE}
...
print (head(sum_columns2,10))
barplot(sum_columns2$value_columns, col = 2,names.arg = sum_columns2$key,xlab="categories")
...
```
Cluster 1 = health and personal fitness
Cluster 2 = college and online gaminng
Cluster 3 = too mixed
Cluster 4 = photo sharing
Cluster 5 = Politics and travel
Cluster 6 = too mixed
```{r}
...
#Cluster 1
qplot(health_nutrition, personal_fitness, data=social_clean3, color=factor(clust1$cluster))

#Cluster 2 - online gaming and college
qplot(online_gaming, college_uni, data=social_clean3, color=factor(clust1$cluster))

#Cluster 3 - mixed
qplot(art, home_and_garden, data=social_clean3, color=factor(clust1$cluster))

#Cluster 4 - photosharing and travel
qplot(photo_sharing, travel, data=social_clean3, color=factor(clust1$cluster))

#Cluster 5 - politics and travel
qplot(politics, travel, data=social_clean3, color=factor(clust1$cluster))

#Cluster 6 - mixed
qplot(family, dating, data=social_clean3, color=factor(clust1$cluster))
qplot(outdoors, automotive, data=social_clean3, color=factor(clust1$cluster))
...
Below are the groupings I would have assumed without doing a statistical analysis. I wanted to force them into new columsn and see what the data looked like afterwards to see if this would manufacture any insights. 
